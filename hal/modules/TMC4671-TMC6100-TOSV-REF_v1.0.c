...
const char *VersionString="0020V111";
...
void tmcm_initMotorConfig()
{
	...
	motorConfig[0].asbThreshold             = 500;
	motorConfig[0].asbVolumeCondition       = 70;
...
void tmcm_updateConfig()
{
	...
	tosvConfig[0].asbVolumeCondition= motorConfig[0].asbVolumeCondition;
...
void tmcm_initModuleSpecificIO()
{
	...
	// outputs port A
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_3 | GPIO_Pin_6 | GPIO_Pin_7 | GPIO_Pin_8 | GPIO_Pin_9 | GPIO_Pin_10 | GPIO_Pin_11 | GPIO_Pin_12 | GPIO_Pin_15;		// CS_Weasel | CS_Dragon | RUN_LED | ERROR_LED | RS485_DIR | EN_Weasel
	...
	GPIOA->BRR = GPIO_Pin_12;			// RS485_DIR
...
uint8_t tmcm_isUartSending(){ return false; }
...
uint8_t tmcm_isRS485Sending()
{
	return (GPIOA->ODR & BIT12);
}
